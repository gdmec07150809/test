...
//				}
			...
			sendMessage() {
				...
				if(mes != "") {
					...
					//console.log(this.$store.state.new_ws.readyState,"状态")
				...
				var that=this;
			     this.$http({
					...
					if(response.body.meta.res == '00000') {
						...
						console.log(that.chatContent,"11111111111111")
						}
		...
		mounted() {
		 ...
		 computed:{
		 	msgs(){
					...
					if(this.$store.state.msgs.fromOpId==this.$store.state.chat_item.fromOpId){
						this.chatContent.push(this.$store.state.msgs);
