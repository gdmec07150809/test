<template>
	...
	-->
		<div style="margin:  3rem 35% 0;width: 30%;text-align: center;" v-if="data.length==0">
			<img src="../../assets/img/icon_noComment.png" style="width: 100%" />
			<p style="margin-top: 0.2rem;color: #CCCCCC; font-size: 0.26rem;">暂时没有新的聊天消息哦！</p>
		</div>
		<div class="chat_box" v-for="item in data">
			...
			<div @click="to_chat(item)" class="text_box">
				...
				<!--用户ID-->
				<p style=" font-size: 0.34rem; margin-top: 0.2rem;font-weight: bold;">{{item.fromNiceName?item.fromNiceName:nink_chuli(item.message)}}</p>
				<!--消息-->
	...
	export default {
		...
		computed: {
			...
			data() {
								...
								for(var i = 0; i < new_arr.length; i++) {
									...
									if(this.ops.length>0){
										for(var j=0;j<this.ops.length;j++){
											/**判断是否是朋友信息,是则显示，不是则不显示*/
											if(this.ops[j].opId==json.fromOpId){
												new_arr2.push(json)
											}
				...
				console.log(new_arr2, "111111111111111")
			return new_arr2;
			}
		}
	}
