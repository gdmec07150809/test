<template>
	...
	<div style="width: 100%;background:white;    overflow: hidden;">
	  <!--<div  v-for="(j,index) in $store.state.shou_cang" style="width:100%;background:white;margin-top: 0.2rem;">-->
	  	...
	  	<!--<div v-show="j.dataArticle.length!=0" style="width:100%;height:0.4rem;">
			...
			{{j.sortDate.substring(8,10)}}日
		     ...
		     <p style="font-size:0.25rem;color:#A9A9A9;float: left;">{{j.dataArticle.length}}件</p>
	  	</div>
		<!--<div style="width: 7.5rem;height:0.4rem;"></div>-->
		
	<!--<div style="width: 100%;overflow: hidden;">
			<div v-for="(i,index) in j.dataArticle" @click="tiao(i)" class="boxss" style="float:left;">
		...
		<div class="box" v-for="(i,index) in $store.state.my_artTle"  @click="tiao(i)">
	  	  <div class="box_s">
	  ...
	  <!--</div>-->
	   ...
	   <div class="no_article" v-if="$store.state.my_artTle.length==0">
	...
	export default{
		...
		methods:{
			...
			tiao(i){
				...
				this.$store.state.tr = 22
				this.$store.state.back_id=93
				this.$router.push({
