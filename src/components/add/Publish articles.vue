<template>
	<div style="width: 100%;height: 100%;position: relative;z-index:300;background: white;" id="">
		...
		<div style="width: 100%;height: 1rem;position:fixed;top:0;left: 0;z-index: 9992;text-align:center;line-height: 0.88rem;border-bottom: 0.005rem solid #f1f1f1;background: white;">
			...
			<p style="font-size: 0.38rem;line-height: 1rem;float:left;margin-left: 2.9rem;">发布热点</p>
		...
		<div style="width: 90%;position: relative;margin: 0 auto;margin-bottom:1.5rem;">

			<!--发文章 -->

			<!--标题 -->
			<!--<h6 style="font-size: 0.5rem;margin-top: 0.2rem;">
		     	热点文章</h6>-->
			...
			<div style="margin-top: 0.3rem;">
				<div style="margin-top: 0.3rem;overflow: hidden;width: 30%;margin-right: 3%;position: relative;" class="zhu" v-for="(i,index) in $store.state.wenzhang.img_boxs">
					<img v-if="i.picUrl" src="static/img/xin/icon_announce_delete.png" style="z-index: 300;position: absolute;top: 0;right: 0;width: 0.5rem;height: 0.5rem;" @click="delete_pic(index)" />
					<img @load="img_cli" id="img" style="height:auto;width:100%;position: absolute;top:50%; left:50%;transform: translate(-50%,-50%);" :src="i.picUrl" />
				</div>
				<!--<img v-for="(i,index) in $store.state.wenzhang.img_boxs" style="margin-right: 0.2rem;margin-top: 0.2rem;" class="zhu" :src="i.picUrl"/>-->
               ...
               <img  class="zhu" src="../../assets/img/drawablefffff/icon_announce_lifeImg.png"/>
				...
				<div style="float:left;margin-top: 0.3rem;width: 30%;height: 30%;position: relative;" v-if="$store.state.wenzhang.img_boxs.length==1?false:true">
			...
			<div @click="Addlabels" style="margin-top: 0.4rem;border-top: 0.01rem solid #F1F1F1;border-bottom: 0.01rem solid #F1F1F1;float: left;" class="boxs">
				...
				<!--
                 ...
                 -->
				<p v-for="(i,index) in $store.state.wenzhang.Label" style="font-size: 0.28rem;float: left;" @click.stop="delete_label(index)">{{i}}&nbsp;&nbsp;&nbsp;</p>
			...
			<!--
             	...
             	描述：点击 ——》添加标签——》出现的临时输入框
             -->
			<div style="position:relative;height:1rem;float:left;margin-top: 0.2rem;" :class="{ji:act.noa,jis:act.noe}">
				<input v-model="text_inp" style="width:70%;height:0.7rem;font-size:0.3rem;border:none;position: absolute;left: 0.2rem;top: 0.2rem;" type="text" placeholder="请输入不超过8个字的标签" maxlength="8" />
			...
			<div style="margin-top: 0.2rem;border-bottom: 0.1rem solid #F1F1F1;float: left;" class="boxs">
				<input v-model="$store.state.wenzhang.Title" style="width: 100%;height: 1rem;font-size: 0.45rem;border:none;float: left;font-weight:600;" type="text" placeholder="文章标题" />
			</div>
		...
		<div style="overflow: hidden;width: 100%;">
			<textarea v-show="pon" id="myInput" v-model="$store.state.wenzhang.Text_content" style="width:90%;font-size: 0.3rem;border:none;margin-left:5%;height:auto;height:6.7rem;margin-top: 0.3rem;" placeholder="正文内容"></textarea>
		</div>
	...
	export default {
		...
		methods: {
			...
			delete_label(i) {
				...
				this.$store.state.wenzhang.Label.splice(i, 1);
				if(this.$store.state.wenzhang.Label.length == 0) {
					this.act.noa1 = true;
					this.act.noe1 = false;
				}
			},
			hui() {
				...
				//当用户点击返回按钮后清空内容
				this.$store.state.wenzhang.img_boxs = []
				this.$store.state.wenzhang.Title = ''
				this.$store.state.wenzhang.Text_content = ''
				this.$store.state.wenzhang.Label = []
				this.act.noa1 = true;
				this.act.noe1 = false
			},
			Addlabels() { //点击添加标签
				if(this.$store.state.wenzhang.Label.length == 3) {
					Toast({
						message: "文章标签只能添加3个",
						position: 'middle',
						duration: 2000
			...
			tijiao() {
				...
				if(this.$store.state.wenzhang.Label !== '') {
					if(this.$store.state.wenzhang.Label.length !== 3) {
						this.act.noa1 = false;
						this.act.noe1 = true
						this.$store.state.wenzhang.Label.push("#" + this.text_inp + '#')
						this.text_inp = ''
						this.act.noa = true;
						this.act.noe = false
					} else {
			...
			/*删除图片*/
			delete_pic(i) {
				this.$store.state.wenzhang.img_boxs.splice(i, 1);
			},
			//图片大小验证  不能超过3M
			verificationPicFile(file) {
				var fileSize = 0;
				var fileMaxSize = 3; //3M  
				var filePath = file;
			...
			uploadpic_tos(e) { //上传图片
				//				if(this.$store.state.wenzhang.img_boxs.length < 9) {
				...
				//						param.append('plateName', 'hot') // 添加form表单中其他数据
				//
				...
				//							//                    	  
				//							//                         this.$store.state.wenzhang.img_boxs.push(response.data.data)
				//							//                         
				//							//                         if(this.$store.state.wenzhang.img_boxs.length!=0){
				//							//                         	  this.supplementary_pic="supplementary_pic"
				//							//                           }
				//							//                         
				//							//                         //console.log(this.$store.state.wenzhang.img_boxs,'asdfadsfhsdjf')
				//							//                         
				//							//                       }
				//							//                    })
				//							this.$http({
				...
				//
				//				}
				if(this.$store.state.wenzhang.img_boxs.length < 1) {
					var url = this.$store.state.request_url + "/api/mapManager/uploadPic.do";
					var id = this.$store.state.data.memId;
					var token = this.$store.state.data.tokEn;
					var ts = new Date().getTime();
					...
					if(aa != null && aa != "") {
						if(this.verificationPicFile(aa)) {
							...
							form.append('plateName', 'hot') // 添加form表单中其他数据
							...
							$.ajax(settings).done(function(abc) {
								...
								if(abc.meta.msg == "ok") {
									that.$store.state.wenzhang.img_boxs.push(abc.data)

									if(that.$store.state.wenzhang.img_boxs.length != 0) {
					...
					if(this.$store.state.wenzhang.Label.length > 0) {
						...
						this.$http({
							...
							body: {
								...
								picUrl: this.$store.state.wenzhang.img_boxs,
								artTitle: this.$store.state.wenzhang.Title,
								docUrl: this.$store.state.wenzhang.Text_content,
								artFlag: '3',
								tags: this.$store.state.wenzhang.Label.toString(),
