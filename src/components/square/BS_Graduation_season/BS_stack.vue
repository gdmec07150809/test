<template>
	<div style="width:100%;">
       ...
       <div class="no_article" v-show="pages.length==0">
			<img src="../../../assets/img/icon_noArticle.png" style="width: 100%;height: 100%;" />
			<p>暂无内容</p>
		</div>
       
		<!--<div  v-show="home_1" style="width: 3.4rem;height: 5rem;margin: 0 auto;margin-top:4rem;font-size: 0.3rem;color: #CCCCCC;">
	  	  <img  style="width:3.4rem;" src="../../../assets/img/icon_noPaint.png" alt="" />
	  	           <p>作品被你滑完了，请稍后再来</p>
	  </div>
	  
	  <div  v-show="home_2" style="width: 3.4rem;height: 5rem;margin: 0 auto;margin-top:4rem;font-size: 0.3rem;color: #CCCCCC;">
	  	   <img style="width: 3.4rem;" src="../../../assets/img/icon_noPaint.png" alt="" />
	  	        <p>搜索不到你想要的作品哦</p>
	  </div>-->

		<ul style="margin-top: -1rem;" :class="{stacks:stack1,stack_1s:stack2,stack_2s:stack3,stack_3s:stack4,stack_4s:stack5}">
			<li @touchmove.stop class="stack-item" v-for="(item, index) in pages" :style="[transformIndex(index),transform(index)]" 
      ...
      @transitionend.stop="onTransitionEnd(index)">
          ...
          <img  style="min-height:100%;min-width:6.85rem;" :src="item.picUrl"/>-->
				...
				<div class="rgbas" style="width: 100%;height: 1.5rem;position:fixed;bottom:0;">
                    ...
                    <div class="p_boxss" style="">
                     	
						<p style="font-size: 0.28rem;color:#B0B0B0;margin-left: 0.31rem;float: left;">{{(item.profession==null)?'':item.profession}}</p>

						<p v-show="true" style="font-size:0.28rem;color:#B0B0B0;margin-left: 0.2rem;float:left;margin-top: -0.01rem;">|</p>

						<p style="font-size:0.28rem;color:#B0B0B0;float: left;margin-left: 0.2rem;">{{(item.artType==null)?'':item.artType}}{{(item.artType==null)?'':item.artType}}</p>
				   </div> 
					
					</div>

					<div @click="dashang(item)" style="width: 1.3rem;height:1.3rem;float:right;margin-right:0.15rem;border-radius: 50%;margin-top: 0.09rem;">
		...
		<!--<div v-show="xian_s" class="zhe"  @touchmove.prevent>
			<div class="zhe_1" style="position: relative;overflow:hidden;">
				...
				<p style="    font-size: 0.35rem;margin-top: 0.45rem;">你还没有完善资料(上传头像)，是否立即去完善资料?</p>
...
<script>
	import detectPrefixes from '../../home/utils/detect-prefixes.js'
...
//						artTitle: '测试',
//						artType: '类型',
//						picUrl: 'http://f10.baidu.com/it/u=2465775762,1509670197&fm=72'
//					},
//					{
//						artFlag: '123',
//						artTitle: '测试',
//						artType: '类型',
//						picUrl: 'http://img4.imgtn.bdimg.com/it/u=1561000227,838375663&fm=27&gp=0.jpg'
//					},
//					{
//						artFlag: '123',
//						artTitle: '测试',
//						artType: '类型',
//						picUrl: 'http://img5.imgtn.bdimg.com/it/u=1706662613,4153832992&fm=27&gp=0.jpg'
//					},
//					{
//						artFlag: '123',
//						artTitle: '测试',
//						artType: '类型',
//						picUrl: 'http://img4.imgtn.bdimg.com/it/u=1837408469,3117811683&fm=27&gp=0.jpg'
//					}
				],
...
//				console.log(this.page_s,'-----页数')
                        console.log(localStorage.yuan_xiao,localStorage.nian_fen,localStorage.lei_xing)
                       
						this.$store.state.Display_storage=[];
						var url=this.$store.state.request_url+"/api/mapPlazaManager/graduationSeasonArtVoteList.do";
				    	...
				    	this.$http({
								url: this.$store.state.request_url+"/api/mapPlazaManager/graduationSeasonArtVoteList.do?memId="+id+"&ts="+ts,
								headers: {
...
//									opId:localStorage.memId,
									page:this.$store.state.page_ss,
									rows:10,
//									max:this.$store.state.max+'',
//									min:this.$store.state.min+'',
//									gander:this.$store.state.gander,
//									typesOf:(this.$store.state.typesOf.length==0)?null:this.$store.state.typesOf
				//					area:this.$store.state.area,
				                    ...
				                    schoolName:(localStorage.yuan_xiao==undefined||localStorage.yuan_xiao=='null')?null:localStorage.yuan_xiao,//学校名称,空值或者null代表查询全部
				                    graduatedYear:(localStorage.nian_fen==undefined||localStorage.nian_fen=='null')?null:localStorage.nian_fen,//毕业年份，空值或者null代表查询全部
				                    artType:(localStorage.lei_xing==undefined||localStorage.lei_xing=='null')?null:localStorage.lei_xing.split(",")//作品分类
								...
								if(response.body.meta.res == '00000'){
				                  ...
				                  for(var i=0;i<response.body.data.dataList.length;i++){
				                  	...
				                  	this.$store.state.Display_storage.push(response.body.data.dataList[i])
				                  }
				                   this.pages = this.$store.state.Display_storage
				                  
//				                  if(this.$store.state.sou_home == 1&&response.body.data.dataList.length==0){
//				                  	
//				                  	 this.home_2 = true
//				                  	 
//				                  }else if(response.body.data.dataList.length==0){
//				                  	
//				                  	 this.home_1 = true
//				                  }
			...
			touchend(e) {
				...
				if(this.offsetRatio >= 0.4) {
					...
					if(this.temporaryData.poswidth > 0) {
						...
						this.tai.na3 = true;
						this.tai.ne3 = false
						if(this.$store.state.pan_duan_s != 2) {
							//console.log(this.pages.length,'数据长度5555555555555555555555555555555555555555555555------')
							this.like(e)
							this.jsq()
						} else {
							this.xian_s = true
...
<style >
  ...
  .stack_4s{/*其他全面屏手机-专属适配*/
	...
	.stack-item {
		ackground: #fff;
		-moz-box-shadow: 0em 1em 1em #f1f1f1;
		-webkit-box-shadow: 0em 1em 1em #f1f1f1;
		box-shadow: 0em 1em 1em #f1f1f1;
		height: 94%;
		width: 91%;
		overflow: hidden;
		position: absolute;
