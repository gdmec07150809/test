...
//				}
			...
			/**选择板块*/
			to_select_article() {
				console.log(this.$store.state.if_depth_articleItem,this.selected_arr)
				this.$store.state.if_depth_articleItem = this.selected_arr
				//this.$store.state.tr = 2;
				this.$router.push({
					path: '../select_depth_article_block'
				});
			},
			tiao_s() {
				let ev = ev || event
...
//							} else {
						...
						if(this.$store.state.if_depth_articleItem.length > 0) {
							...
							for(var i = 0; i < this.$store.state.if_depth_articleItem.length; i++) {
								for(var j = 0; j < new_array.length; j++) {
									if(this.$store.state.if_depth_articleItem[i] == new_array[j].dictName) {
										new_array[j].id = true
									}
								}
							}
		...
		mounted() {
			...
			if(this.$store.state.if_depth_articleItem.length == 0) {
				...
				for(var i = this.$store.state.if_depth_articleItem.length - 1; i >= 0; i--) {
					this.selected_arr.push(this.$store.state.if_depth_articleItem[i])
				}
			}
			this.getZhuanye();
			if(localStorage.yous=='true'){
